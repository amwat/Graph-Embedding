...
def records_to_graph():
    ...
    for movie in movies:
        for i in xrange(1, 6):
            name = movie + '_' + str(i)
            node = Node(idx, name, 'movie-rating')
            nodedict[name] = node
            nodelist.append(node)
            idx += 1
    ...
    for user in ratings:
        for movie, rating in ratings[user].items():
            name = movie + '_' + str(rating)
            nodedict[user].neighbors.append(nodedict[name])
            nodedict[name].neighbors.append(nodedict[user])

    # Add edges between movies and directors
    for movieID, movie in movies.items():
        if movie.director:
            nodedict[movieID].neighbors.append(nodedict[movie.director])
            nodedict[movie.director].neighbors.append(nodedict[movieID])

    # Add edges between movies and actors
    for movieID, movie in movies.items():
        for actor in movie.actors:
            nodedict[movieID].neighbors.append(nodedict[actor])
            nodedict[actor].neighbors.append(nodedict[movieID])
    # Add edges between movies and genres
    for movieID, movie in movies.items():
        for genre in movie.genres:
            nodedict[movieID].neighbors.append(nodedict[genre])
            nodedict[genre].neighbors.append(nodedict[movieID])
    # Add edges between movie ratings and movies
    for movie in movies:
        for i in xrange(1, 6):
            name = movie + '_' + str(i)
            nodedict[movie].neighbors.append(nodedict[name])
            nodedict[name].neighbors.append(nodedict[movie])
